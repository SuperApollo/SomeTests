...
//    }
    ...
    private void initView(int height, int widthMeasureSpec, int heightMeasureSpec) {
        LampView.this.setOrientation(VERTICAL);
        if (lights.size() > 0) {
            LampView.this.removeAllViews();
            lights.clear();
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            LampView.this.addView(imageView, params);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case LOOP_TYPE_SINGLE:
                    mMyHandler.removeMessages(LOOP_TYPE_SINGLE);//用完一次移除一次message
